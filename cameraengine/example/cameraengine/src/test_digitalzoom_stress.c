...
int _test_digitalzoom_stress( CAM_DeviceHandle hHandle )
{
	CAM_Error              error;
	...
	error = CAM_SendCommand( hHandle, CAM_CMD_PORT_SET_SIZE, (CAM_Param)CAM_PORT_PREVIEW, (CAM_Param)&stPreviewSize );
	ASSERT_CAM_ERROR( error );
	
	StartPreview( hHandle );

	error = CAM_SendCommand( hHandle, CAM_CMD_GET_SHOTMODE, (CAM_Param)&eShotMode, UNUSED_PARAM );
	ASSERT_CAM_ERROR( error );

	error = CAM_SendCommand( hHandle, CAM_CMD_QUERY_SHOTMODE_CAP, (CAM_Param)eShotMode, (CAM_Param)&stShotModeCap );
	ASSERT_CAM_ERROR( error );
	...
	for ( i = 0; i < loop; i++ )
	{
		...
		for ( j = 0; j <= ( stShotModeCap.iMaxDigZoomQ16 - stShotModeCap.iMinDigZoomQ16 ) / stShotModeCap.iDigZoomStepQ16; j++ )
		{
			...
			error = CAM_SendCommand( hHandle, CAM_CMD_SET_DIGZOOM, (CAM_Param)iDigitalZoomQ16, UNUSED_PARAM );
			ASSERT_CAM_ERROR( error );
...
int test_digitalzoom_stress( CAM_DeviceHandle hHandle )
{
	int val;
	...
	*/
	...
	val = _test_digitalzoom_stress( hHandle );
