...
#define ALIGN_TO(x, iAlign) ( (((CAM_Int32s)(x)) + (iAlign) - 1) & (~((iAlign) - 1)) )
...
#define ASSERT_CAM_ERROR( err )\
	...
	case CAM_ERROR_BUFFERUNAVAILABLE:\
		TRACE( "Error Code: CAM_ERROR_BUFFERUNAVAILABLE, %s - %d\n", __FILE__, __LINE__ );\
		break;\
...
#define APP_VIDEO_BUFCNT          8 // (APP_VIDEO_ENCODER_BUFCNT + APP_CAMERA_BUFCNT + APP_SWITCH_BUFCNT - 1)
...
typedef enum
{
	...
	CMD_BANDFILTER,
	CMD_SENSORFLIP,
	CMD_STARTFD,   // start face detection
	CMD_CANCELFD,  // stop face detection
} CAM_AppCmd;
...
extern CAM_Error RestartCameraEngine(CAM_DeviceHandle hHandle);
extern CAM_Error SafeDequeue(CAM_DeviceHandle hHandle, int *pPort, CAM_ImageBuffer **ppImgBuf);

extern CAM_Error InitEngine( CAM_DeviceHandle *pHandle );
extern CAM_Error SetSensor( CAM_DeviceHandle hHandle, CAM_Int32s iSensorID );
extern CAM_Error DeinitEngine(CAM_DeviceHandle hHandle);

extern CAM_Error StartPreview( CAM_DeviceHandle hHandle );
extern CAM_Error StopPreview( CAM_DeviceHandle hHandle );

extern CAM_Error StartRecording( CAM_DeviceHandle hHandle );
extern CAM_Error StopRecording( CAM_DeviceHandle hHandle );

extern CAM_Error AutoFocus( CAM_DeviceHandle hHandle );
extern CAM_Error CancelAutoFocus( CAM_DeviceHandle hHandle );
extern CAM_Error TakePicture( CAM_DeviceHandle hHandle );

extern CAM_Error SetParameters( CAM_DeviceHandle hHandle, CAM_AppCmd cmd, CameraParam param );
extern CAM_Error GetParametersAndDisplay( CAM_DeviceHandle hHandle, CAM_CameraCapability *pCameraCap );

extern CAM_Error RunOneFrame( CAM_DeviceHandle hHandle );
extern CAM_Error DequeueUserCmd( CAM_DeviceHandle hHandle, CAM_AppCmd *pCmd, CameraParam *pParam) ;
extern CAM_Error DeliverPreviewBuffer( CAM_DeviceHandle hHandle, CAM_ImageBuffer *pImgBuf );
extern CAM_Error DeliverSnapshotBuffer( CAM_DeviceHandle hHandle, CAM_ImageBuffer *pImgBuf );
extern CAM_Error DeliverVideoBuffer( CAM_DeviceHandle hHandle, CAM_ImageBuffer *pImgBuf );
extern CAM_Error DeliverStillBuffer( CAM_DeviceHandle hHandle, CAM_ImageBuffer *pImgBuf );
