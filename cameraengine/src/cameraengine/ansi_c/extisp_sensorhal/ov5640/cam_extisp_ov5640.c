...
{
	{ OV5640_REG_EM_WIN00WEI,0x01},
	{ OV5640_REG_EM_WIN01WEI,0x10},
	{ OV5640_REG_EM_WIN02WEI,0x01},
	{ OV5640_REG_EM_WIN03WEI,0x10},
...
{
	{ OV5640_REG_EM_WIN00WEI,0x00},
	{ OV5640_REG_EM_WIN01WEI,0x00},
	{ OV5640_REG_EM_WIN02WEI,0x00},
	{ OV5640_REG_EM_WIN03WEI,0x00},
...
{
	{ OV5640_REG_EM_WIN00WEI,0x02},
	{ OV5640_REG_EM_WIN01WEI,0x60},
	{ OV5640_REG_EM_WIN02WEI,0x06},
	{ OV5640_REG_EM_WIN03WEI,0x20},
...
/*
static CAM_Error _OV5640SaveAeAwb( void* );
static CAM_Error _OV5640RestoreAeAwb( void* );
...
extern _CAM_SmartSensorFunc func_ov5640;
CAM_Error OV5640_SelfDetect( _CAM_SmartSensorAttri *pSensorInfo )
{
...
#else
	{
		...
		strcpy( pSensorInfo->stSensorProp.sName, "ov5640" );
		pSensorInfo->stSensorProp.eInstallOrientation = CAM_FLIPROTATE_NORMAL;
		pSensorInfo->stSensorProp.iFacing             = CAM_SENSOR_FACING_BACK;
		pSensorInfo->stSensorProp.iOutputProp         = CAM_OUTPUT_SINGLESTREAM;
...
CAM_Error OV5640_Init( void **ppDevice, void *pSensorEntry )
{
	...
	*ppDevice = pState;
	...
	_OV5640Attri.iUnstableFrameCnt    = 20;
...
#else
	_OV5640Attri.iUnstableFrameCnt    = 2;
	...
	error = V4L2_Init( &pState->stV4L2, &_OV5640Attri );
...
CAM_Error OV5640_Deinit( void **ppDevice )
{
	OV5640State *pState = (OV5640State*)(*ppDevice);
	CAM_Error   error   = CAM_ERROR_NONE;
...
#if 0
	// ISO setting
	if ( pShotParam->eIsoMode != pState->stV4L2.stShotParamSetting.eIsoMode )
	{
		for ( i = 0; i < _ARRAY_SIZE(_OV5640IsoMode); i++ )
		{
			if ( pShotParam->eIsoMode == (CAM_ISOMode)_OV5640IsoMode[i].iParameter )
			{
				break;
			}
		}
		...
		pState->stV4L2.stShotParamSetting.eIsoMode = pShotParam->eIsoMode;

	}

	// Color Effect Setting
	if ( pShotParam->eColorEffect != pState->stV4L2.stShotParamSetting.eColorEffect )
	{
		for ( i = 0; i < _ARRAY_SIZE(_OV5640ColorEffectMode); i++ )
		{
			if ( pShotParam->eColorEffect == (CAM_ColorEffect)_OV5640ColorEffectMode[i].iParameter )
			{
				break;
			}
		}
		...
		pState->stV4L2.stShotParamSetting.eColorEffect = pShotParam->eColorEffect;
	}

	// White Balance Setting
	if ( pShotParam->eWBMode != pState->stV4L2.stShotParamSetting.eWBMode )
	{
		for ( i = 0; i < _ARRAY_SIZE(_OV5640WBMode); i++ )
		{
			if ( pShotParam->eWBMode == (CAM_WhiteBalanceMode)_OV5640WBMode[i].iParameter )
			{
				break;
			}
		}
		...
		pState->stV4L2.stShotParamSetting.eWBMode = pShotParam->eWBMode;
	}


	// Exposure Metering mode
	if ( pShotParam->eExpMeterMode != pState->stV4L2.stShotParamSetting.eExpMeterMode )
	{
		for ( i = 0; i < _ARRAY_SIZE(_OV5640ExpMeterMode); i++ )
		{
			if ( pShotParam->eExpMeterMode == (CAM_ExposureMeterMode)_OV5640ExpMeterMode[i].iParameter )
			{
				break;
			}
		}
		...
		pState->stV4L2.stShotParamSetting.eExpMeterMode = pShotParam->eExpMeterMode;
	}

	// Shutter Speed setting
	if ( pShotParam->iShutterSpeedQ16 != pState->stV4L2.stShotParamSetting.iShutterSpeedQ16 )
	{
		;
	}

	// still sub-mode
	if ( pShotParam->eStillSubMode != pState->stV4L2.stShotParamSetting.eStillSubMode ||
	     pShotParam->iBurstCnt != pState->stV4L2.stShotParamSetting.iBurstCnt ||
	     pShotParam->iZslDepth != pState->stV4L2.stShotParamSetting.iZslDepth )
	{
		CELOG( "Still sub-mode\n" );
		// TODO: if your sensor/ISP originally support burst/zsl, set ISP sepecific params here

		pState->stV4L2.stShotParamSetting.eStillSubMode = pShotParam->eStillSubMode;
		pState->stV4L2.stShotParamSetting.iBurstCnt = pShotParam->iBurstCnt;
		pState->stV4L2.stShotParamSetting.iZslDepth = pShotParam->iZslDepth;
	}
...
CAM_Error OV5640_StartFocus( void *pDevice )
{
...
CAM_Error OV5640_CheckFocusState( void *pDevice, CAM_Bool *pFocusAutoStopped, CAM_FocusState *pFocusState )
{
...
CAM_Error OV5640_StartFaceDetection( void *pDevice )
{
...
_CAM_SmartSensorFunc func_ov5640 =
{
...
static CAM_Error _ov5640_save_ae_awb( CAM_Int32s iSensorFD, _OV5640_3AState *pCurrent3A )
{
	...
	/* VTS */
	reg = 0;
	_get_sensor_reg( iSensorFD, OV5640_REG_AECVTSHIGH, &reg );
	maxLines = ( reg  & 0x00ff );
	maxLines <<= 8;

	reg = 0;
	_get_sensor_reg( iSensorFD, OV5640_REG_AECVTSLOW, &reg );
	maxLines += ( reg & 0x00ff );
...
static CAM_Error _ov5640_calc_new_3A( CAM_Int32s iSensorFD, _OV5640_3AState *p3Astat )
{
	...
	*/
	...
	// calculate capture exposure, in unit of t_ROW_INTERVAL
...
static CAM_Error _OV5640SaveAeAwb( void *pUserData )
{
	CAM_Error   error   = CAM_ERROR_NONE;
...
static CAM_Error _OV5640RestoreAeAwb( void *pUserData )
{
	CAM_Error   error   = CAM_ERROR_NONE;
	OV5640State *pState = (OV5640State*)pUserData;
