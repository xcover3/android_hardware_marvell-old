...
CAM_Error FakeSensor_SelfDetect( _CAM_SmartSensorAttri *pSensorInfo )
{
	CAM_Error error = CAM_ERROR_NONE;

	_V4L2SensorEntry *pSensorEntry = (_V4L2SensorEntry*)( pSensorInfo->cReserved );
	strcpy( pSensorInfo->stSensorProp.sName, "BasicSensor-fake" );
...
CAM_Error FakeSensor_Deinit( void **ppDevice )
{
	CAM_Error       error   = CAM_ERROR_NONE;
	FakeSensorState *pState = (FakeSensorState*)(*ppDevice);
...
CAM_Error FakeSensor_StartFocus( void *pDevice )
{
	return CAM_ERROR_NONE;
}
...
CAM_Error FakeSensor_CheckFocusState( void *pDevice, CAM_Bool *pFocusAutoStopped, CAM_FocusState *pFocusState )
{
...
CAM_Error FakeSensor_StartFaceDetection( void *pDevice )
{
	return CAM_ERROR_NONE;
}

CAM_Error FakeSensor_CancelFaceDetection( void *pDevice )
{
	return CAM_ERROR_NONE;
}

_CAM_SmartSensorFunc func_fakesensor =
{
	...
	FakeSensor_StartFaceDetection,
	FakeSensor_CancelFaceDetection,
};
