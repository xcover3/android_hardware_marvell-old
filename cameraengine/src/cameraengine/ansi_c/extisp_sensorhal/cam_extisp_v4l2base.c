...
#define CAM_V4L2_DEVICE_NUM   1
#define CAM_V4L2_CHANNEL_NUM  1
                           ...
                           CAM_ImageFormat eVideoFormatTable[CAM_MAX_SUPPORT_IMAGE_FORMAT_CNT], CAM_ImageFormat eStillFormatTable[CAM_MAX_SUPPORT_IMAGE_FORMAT_CNT] )
{
	...
	static CAM_Int8u        sDevicePool[CAM_V4L2_DEVICE_NUM][12] = { "/dev/video0" };
	static CAM_Int32s       bDeviceOccupy[CAM_V4L2_DEVICE_NUM][CAM_V4L2_CHANNEL_NUM] = { {0, 0} }; // 0: idle; 1: invalid; 2: occupied
	CAM_Size                stVideoSizeTab[CAM_MAX_SUPPORT_IMAGE_SIZE_CNT]   = { {0, 0}, };
	CAM_Int32s              iVideoSizeCntTab[CAM_MAX_SUPPORT_IMAGE_SIZE_CNT] = { 0, };

	memset( pSensorInfo, 0, sizeof( _CAM_SmartSensorAttri ) );

	for ( i = 0; i < CAM_V4L2_DEVICE_NUM; i++ )
	{
		CAM_Int32s fd              = -1;
		CAM_Bool   bDeviceOccupied = CAM_TRUE;
		...
		for ( j = 0; j < CAM_V4L2_CHANNEL_NUM; j++ )
		{
			...
			{
				bDeviceOccupied = CAM_FALSE;
			}
		}
		if ( bDeviceOccupied == CAM_TRUE )
		{
			TRACE( CAM_INFO, "Info: device[%s] has been occupied or invalid( %s, %d )\n", sDevicePool[i], __FILE__, __LINE__ );
			continue;
		}

		fd = cam_open( (const char*)sDevicePool[i], O_RDWR );
		if ( fd < 0 )
		{
		...
		for ( j = 0; j < CAM_V4L2_CHANNEL_NUM; j++ )
		{
			...
			if ( 0 != ret )
			{
				bDeviceOccupy[i][channel] = 1;
				TRACE( CAM_WARN, "!!!Strongly Warning: channel[%d] of device node %s query capability failed, take it as invalid, pls check if this is a camera sensor, and check VIDIOC_QUERYCAP cmd implementation\
				       in driver( %s, %d )\n", channel, sDevicePool[i], __FILE__, __LINE__ );
				continue;
			}

            ALOGE("capability is: %x", stV4L2DriverCap.capabilities);
            ALOGE("driver name is: %s", stV4L2DriverCap.driver);
			...
			if ( strcmp( sSensorName, "basicsensor" ) )
			{
				...
				{
					strcpy( pSensorInfo->stSensorProp.sName, (const char*)stV4L2DriverCap.driver );
				}
			}
			else
			{
				if ( !strncmp( (const char*)stV4L2DriverCap.driver, "mvsocisp", 8 ) )
				{
					TRACE( CAM_INFO, "Info: channel[%d] of device node %s is a raw camera( %s, %d )\n", channel, sDevicePool[i], __FILE__, __LINE__ );
					bDeviceOccupy[i][j] = 1;
					continue;
				}

				strcpy( pSensorInfo->stSensorProp.sName, "BasicSensor-" );
				if ( ret < 0 || (stV4L2DriverCap.driver)[0] == '\0' )
				{
					strcat( pSensorInfo->stSensorProp.sName, "unknown" );
				}
				else
				{
					strncat( pSensorInfo->stSensorProp.sName, (const char*)stV4L2DriverCap.driver, 15 );
				}
			}
			...
			do
			{
				CAM_Size *pstCurrentSizeTable = NULL;
				...
				case V4L2_PIX_FMT_YUV420:
					...
					pstCurrentSizeTable             = stVideoSizeTab;
					stV4L2FrmSize.pixel_format      = stV4L2Format.pixelformat;
					iVideoFmtCnt++;
					break;

				case V4L2_PIX_FMT_YVU420:
					eVideoFormatTable[iVideoFmtCnt] = CAM_IMGFMT_YCrCb420P;
					pstCurrentSizeTable             = stVideoSizeTab;
					stV4L2FrmSize.pixel_format      = stV4L2Format.pixelformat;
					iVideoFmtCnt++;
					break;
				...
				case V4L2_PIX_FMT_JPEG:
					...
					pstCurrentSizeTable             = stStillResTable;
					stV4L2FrmSize.pixel_format      = stV4L2Format.pixelformat;
					iStillFmtCnt++;
#else
				...
				default:
					TRACE( CAM_ERROR, "Error: unrecognized V4L2 pixel format[0x%x]( %s, %s, %d )\n",
					       stV4L2Format.pixelformat, __FILE__, __FUNCTION__, __LINE__ );
				...
				do
				{
					...
					{
						pstCurrentSizeTable[cnt].iWidth  = stV4L2FrmSize.discrete.width;
						pstCurrentSizeTable[cnt].iHeight = stV4L2FrmSize.discrete.height;
						cnt++;
					}
					else
					{
						...
						{
							if ( pstCurrentSizeTable[k].iWidth == (CAM_Int32s)stV4L2FrmSize.discrete.width &&
							     pstCurrentSizeTable[k].iHeight == (CAM_Int32s)stV4L2FrmSize.discrete.height )
							{
								iVideoSizeCntTab[k]++;
								break;
							}
							...
							{
								if ( pstCurrentSizeTable[k].iWidth != 0 && pstCurrentSizeTable[k].iHeight != 0 )
								{
									continue;
								}
								else
								{
									pstCurrentSizeTable[k].iWidth  = stV4L2FrmSize.discrete.width;
									pstCurrentSizeTable[k].iHeight = stV4L2FrmSize.discrete.height;
									iVideoSizeCntTab[k]++;
									break;
								}
							}
			...
			if ( iVideoFmtCnt != 0 )
			{
				pSensorInfo->pFunc = pCallFunc;

				pSensorEntry = (_V4L2SensorEntry*)(pSensorInfo->cReserved);

				strcpy( pSensorEntry->sDeviceName, (const char*)sDevicePool[i] );
			...
			{
				bDeviceOccupy[i][channel] = 1;
				TRACE( CAM_WARN, "!!!Strongly Warning: channel[%d] of device node %s enum format is 0, take it as invalid, pls check if this is a camera sensor, and check VIDIOC_ENUM_FMT cmd implementation\
				       in driver( %s, %d )\n", channel, sDevicePool[i], __FILE__, __LINE__ );
				continue;
			}
...
CAM_Error V4L2_Init( _V4L2SensorState *pSensorState, _V4L2SensorAttribute *pSensorAttri )
{
	...
	pSensorState->iSensorFD = cam_open( pSensorAttri->stV4L2SensorEntry.sDeviceName, O_RDWR );
	if ( pSensorState->iSensorFD < 0 )
	{
		TRACE( CAM_ERROR, "Error: V4L2 failed to open device[%s], error code[%s]( %s, %s, %d )\n",
		       pSensorAttri->stV4L2SensorEntry.sDeviceName, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
	...
	/* if ( 0 != ret ) */
	/* { */
		/* TRACE( CAM_ERROR, "Error: V4L2 set input[sensor id: %d] failed, error code[%s]( %s, %s, %d )\n", */
			   /* pSensorAttri->stV4L2SensorEntry.iV4L2SensorID, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ ); */
		/* return CAM_ERROR_DRIVEROPFAILED; */
	/* } */
	...
	pSensorState->bIsCaptureFlashOn = CAM_FALSE;
	...
	// init remain skip frame count 
	pSensorState->iRemainSkipCnt   = pSensorAttri->iUnstableFrameCnt;
...
CAM_Error V4L2_Enqueue( _V4L2SensorState *pSensorState, CAM_ImageBuffer *pImgBuf )
{
	...
	if ( !bNewBuffer )
	{
		...
		if ( ret == -1 )
		{
			TRACE( CAM_ERROR, "Error: V4L2 enqueue buffer failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
	...
	else
	{
		...
		if ( _is_format_rawandplanar( pImgBuf->eFormat ) )
		{
			CAM_Int32s i = 1;
			while ( ( pImgBuf->iAllocLen[i] > 0 ) && ( i < 3 ) )
			{
				if ( pImgBuf->pBuffer[i] + pImgBuf->iOffset[i] != pImgBuf->pBuffer[i - 1] + pImgBuf->iOffset[i - 1] + pImgBuf->iAllocLen[i - 1] )
				{
	...
	if ( !pSensorState->bIsStreamOn && pSensorState->iV4L2QueueBufCnt >= pSensorState->stBufReq.iMinBufCount )
	{
		// stream on
		...
		if ( 0 != ret )
		{
			TRACE( CAM_ERROR, "Error: V4L2 stream on sensor failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
			return CAM_ERROR_DRIVEROPFAILED;
		}
...
static CAM_Error _V4L2_Dequeue( _V4L2SensorState *pSensorState, CAM_ImageBuffer **ppImgBuf )
{
	...
	{
		TRACE( CAM_ERROR, "Error: Fatal error happened! Please reset camera to recover( %s, %s, %d )!\n", __FILE__, __FUNCTION__, __LINE__ );
	...
	do
	{
		...
		if ( cnt >= 4 && ret <= 0 )
		{
			TRACE( CAM_ERROR, "Error: V4L2 polling failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
	...
	if ( ret != 0 )
	{
		TRACE( CAM_ERROR, "Error: V4L2 VIDIOC_DQBUF failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}

	ASSERT( (CAM_Int32s)stBufNode.index < pSensorState->iV4L2BufCnt );
	...
	*ppImgBuf = pSensorState->stV4L2Buf[stBufNode.index].pImage;
	...
	(*ppImgBuf)->tTick = CAM_TimeGetTickCount() * 1000;
...
CAM_Error V4L2_DequeueWithSkip( _V4L2SensorState *pSensorState, CAM_ImageBuffer **ppImgBuf )
{
	...
	for ( ; ; )
	{
		...
		if ( pSensorState->iRemainSkipCnt <= 0 )
		{
#if defined( CAM_PERF )
			...
			if ( CAM_IMGFMT_JPEG == pSensorState->stConfig.eFormat )
			{
				...
				{
					CELOG( "bad jpeg SOI, skip one more frame( %s, %s, %d )\n", __FILE__, __FUNCTION__, __LINE__ );
					pSensorState->iRemainSkipCnt++;
				}
			}
			else
			{
...
//		}
		...
		if ( pSensorState->iRemainSkipCnt == pSensorState->stAttribute.iUnstableFrameCnt )
		{
			TRACE( CAM_INFO, "Info: Skip %d frames due to sensor stream off/on\n", pSensorState->stAttribute.iUnstableFrameCnt );
		}
	     ...
	     pSensorState->stConfig.eState == CAM_CAPTURESTATE_VIDEO )
	{
		for ( ; ; )
		{
			...
			if ( pSensorState->iValidFrameIndex > 0 )
			{
				...
				target_n_frame_interval = (CAM_Tick)( 65536.0 * 1000000.0 / pSensorState->stShotParamSetting.iFpsQ16 * pSensorState->iValidFrameIndex );

				target_n_minus_1_frame_interval = (CAM_Tick)( 65536.0 * 1000000.0 / pSensorState->stShotParamSetting.iFpsQ16 * ( pSensorState->iValidFrameIndex - 1 ) );
				...
				if ( sensor_n_frame_interval <= target_n_frame_interval && sensor_n_frame_interval > target_n_minus_1_frame_interval )
				{
					if ( pSensorState->iValidFrameIndex % 200 == 0 )
					{
						...
						pSensorState->iActualFpsQ16 = (CAM_Int32s)( 1000000.0 * 65536.0 / sensor_n_frame_interval * pSensorState->iValidFrameIndex );
					}
			...
			{
				pSensorState->tFramesTickStart = tick;
				pSensorState->iActualFpsQ16 = pSensorState->stShotParamSetting.iFpsQ16;
				pSensorState->iValidFrameIndex++;
			}
...
CAM_Error V4L2_Flush( _V4L2SensorState *pSensorState )
{
	...
	if ( pSensorState->bIsStreamOn )
	{
		...
		if ( 0 != ret )
		{
			TRACE( CAM_ERROR, "Error: V4L2 stream off sensor failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
			return CAM_ERROR_DRIVEROPFAILED;
		}
	...
	if ( 0 != ret )
	{
		TRACE( CAM_ERROR, "Error: V4L2 request buffers 0 failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}
	...
	if ( 0 != ret )
	{
		TRACE( CAM_ERROR, "Error: V4L2 close camera sensor[%s] failed, error code[%s]( %s, %s, %d )\n",
		       pSensorState->stAttribute.sV4L2SensorEntry.sDeviceName, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}
	pSensorState->iSensorFD = -1;

	pSensorState->iSensorFD = cam_open( pSensorState->stAttribute.stV4L2SensorEntry.sDeviceName, O_RDWR );
	if ( pSensorState->iSensorFD < 0 )
	{
		TRACE( CAM_ERROR, "Error: V4L2 open sensor device[%s] failed, error code[%s]( %s, %s, %d )\n",
		       pSensorState->stAttribute.sV4L2SensorEntry.sDeviceName, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}
	...
	/* { */
		/* TRACE( CAM_ERROR, "Error: V4L2 set input[sensor id: %d] failed, error code[%s]( %s, %s, %d )\n", */
			   /* pSensorState->stAttribute.stV4L2SensorEntry.iV4L2SensorID, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ ); */
		/* return CAM_ERROR_DRIVEROPFAILED; */
	/* } */
	...
	if ( 0 != ret )
	{
		TRACE( CAM_ERROR, "Error: V4L2 request buffers failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}
	...
	// XXX: leave skip frame interface in flush operation, since for some sensor module,
	// even not configure sensor, just flush, the first several frames may be bad when view on.
	// you can use this interface to skip the bad frames.
	// please note skip frames may infulence view on latence, if much frames needed,
	// please contact sensor manufacturer to check why.

	// pSensorState->iRemainSkipCnt = pSensorState->stAttribute.iUnstableFrameCnt;
...
CAM_Error V4L2_Config( _V4L2SensorState *pSensorState, _CAM_SmartSensorConfig *pSensorConfig )
{
	     ...
	     pSensorConfig->eState         == CAM_CAPTURESTATE_STILL )
	{
		...
		if ( pSensorState->stAttribute.fnStartFlash != NULL )
		{
			...
			// keep capture flash status for exif info
			pSensorState->bIsCaptureFlashOn = pSensorState->bIsFlashOn;
		}
		...
		{
			error = pSensorState->stAttribute.fnSaveAeAwb( pSensorState->stAttribute.pSaveRestoreUserData );
			if ( error != CAM_ERROR_NONE )
			{
				return error;
			}
	...
	if ( pSensorState->stConfig.eState != CAM_CAPTURESTATE_IDLE )
	{
		...
		if ( 0 != ret )
		{
			TRACE( CAM_ERROR, "Error: V4L2 request buffer 0 failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
			return CAM_ERROR_DRIVEROPFAILED;
		}
		...
		if ( pSensorConfig->iResetType == CAM_RESET_COMPLETE )
		{
			...
			{
				TRACE( CAM_ERROR, "Error: V4L2 close camera sensor[%s] failed, error code[%s]( %s, %s, %d )\n",
				       pSensorState->stAttribute.stV4L2SensorEntry.sDeviceName, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
				return CAM_ERROR_DRIVEROPFAILED;
			}
			pSensorState->iSensorFD = -1;
			...
			/* if ( 0 != ret ) */
			/* { */
				/* TRACE( CAM_ERROR, "Error: V4L2 set input[%d] failed, error code[%s]( %s, %s, %d )\n", */
					   /* pSensorState->stAttribute.stV4L2SensorEntry.iV4L2SensorID, strerror( errno ), __FILE__, __FUNCTION__, __LINE__ ); */
				/* return CAM_ERROR_DRIVEROPFAILED; */
			/* } */
		}
	...
	if ( pSensorConfig->eState != CAM_CAPTURESTATE_IDLE )
	{
		...
		if ( 0 != ret )
		{
			TRACE( CAM_ERROR, "Error: V4L2 request buffer failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
			return CAM_ERROR_DRIVEROPFAILED;
		}
	...
	if ( ( pSensorState->stConfig.eState == CAM_CAPTURESTATE_PREVIEW && pSensorConfig->eState == CAM_CAPTURESTATE_STILL ) ||
	     ( pSensorState->stShotParamSetting.eExpMode == CAM_EXPOSUREMODE_MANUAL ) )
	{
		if ( pSensorState->stAttribute.fnRestoreAeAwb != NULL )
		{
			error = pSensorState->stAttribute.fnRestoreAeAwb( pSensorState->stAttribute.pSaveRestoreUserData );
			if ( error != CAM_ERROR_NONE )
			{
				return error;
			}
...
CAM_Error V4L2_GetBufReq( _V4L2SensorState *pSensorState, _CAM_SmartSensorConfig *pSensorConfig, CAM_ImageBufferReq *pBufReq )
{
	...
	// 32 byte alignment is required by DMA
	pBufReq->iAlignment[0] = 32;
	if ( pSensorConfig->eFormat == CAM_IMGFMT_YCbCr444P || pSensorConfig->eFormat == CAM_IMGFMT_YCbCr422P ||
	     pSensorConfig->eFormat == CAM_IMGFMT_YCbCr420P || pSensorConfig->eFormat == CAM_IMGFMT_YCrCb420P )
	{
		// 32 byte alignment is required by DMA
...
CAM_Error V4L2_FirmwareDownload( _V4L2SensorState *pSensorState )
{
	...
	if ( ret != 0 )
	{
		TRACE( CAM_ERROR, "Error: camera firmware download failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return CAM_ERROR_DRIVEROPFAILED;
	}

	TRACE( CAM_INFO, "Info: camera firmware download success( %s, %s, %d )\n", __FILE__, __FUNCTION__, __LINE__ );
	return CAM_ERROR_NONE;
}
...
CAM_Int32s _get_sensor_reg( CAM_Int32s iSensorFD, CAM_Int16u reg, CAM_Int32s lbit, CAM_Int32s hbit, CAM_Int32s val_lbit, CAM_Int16u *pVal )
{
	...
	{
		TRACE( CAM_ERROR, "Error: V4L2 get register value failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
		return 1;
	}
...
CAM_Int32s _set_sensor_reg( CAM_Int32s iSensorFD, CAM_Int16u reg, CAM_Int32s lbit, CAM_Int32s hbit, CAM_Int32s val_lbit, CAM_Int16u val )
{
	...
	if ( s1.val != s.val )
	{
		...
		if ( 0 != ret )
		{
			TRACE( CAM_ERROR, "Error: V4L2 set register value failed, error code[%s]( %s, %s, %d )\n", strerror( errno ), __FILE__, __FUNCTION__, __LINE__ );
			return 1;
		}
#if 0
		{
			...
			{
				ASSERT( val1 == val );
				TRACE( CAM_ERROR, "Error: %s failed: 0x%x[%d:%d] <-- 0x%x[%d:%d]\n", 
				       __FUNCTION__, reg, lbit, hbit, val, val_lbit, val_lbit + hbit - lbit );
