...
#if defined( LOG_TAG )
// #warning LOG_TAG has defined before will redefine it to MrvlCameraEngine
#undef LOG_TAG
...
// function tag
#define FUNC_TAG_E  CELOG( "Enter %s( %s, %d )\n", __FUNCTION__, __FILE__, __LINE__ );
#define FUNC_TAG_X  CELOG( "Exit  %s( %s, %d )\n", __FUNCTION__, __FILE__, __LINE__ );
...
#define _CHECK_BAD_ROI_COORDINATE( pMultiROI )\
	do {\
		CAM_Int32s i;\
		if ( pMultiROI->iROICnt <= 0 )\
		{\
			return CAM_ERROR_BADARGUMENT;\
		}\
		for ( i = 0; i < pMultiROI->iROICnt; i++ )\
		{\
			if ( abs( pMultiROI->stWeiRect[i].stRect.iLeft ) > CAM_NORMALIZED_FRAME_WIDTH / 2 ||\
			     abs( pMultiROI->stWeiRect[i].stRect.iTop ) > CAM_NORMALIZED_FRAME_HEIGHT / 2 ||\
			     abs( pMultiROI->stWeiRect[i].stRect.iLeft + pMultiROI->stWeiRect[i].stRect.iWidth ) > CAM_NORMALIZED_FRAME_WIDTH / 2 ||\
			     abs( pMultiROI->stWeiRect[i].stRect.iTop + pMultiROI->stWeiRect[i].stRect.iHeight ) > CAM_NORMALIZED_FRAME_HEIGHT / 2 )\
			{\
				TRACE( CAM_ERROR, "Error: [%d]th ROI is our of range, please check: left[%d], top[%d], width[%d], height[%d], each ROI coordinate should be normalized by( %d x %d )frame size( %s, %d )\n", \
				       i, pMultiROI->stWeiRect[i].stRect.iLeft,  pMultiROI->stWeiRect[i].stRect.iTop, pMultiROI->stWeiRect[i].stRect.iWidth, pMultiROI->stWeiRect[i].stRect.iHeight, CAM_NORMALIZED_FRAME_WIDTH, CAM_NORMALIZED_FRAME_HEIGHT, __FILE__, __LINE__ );\
				return CAM_ERROR_BADARGUMENT;\
			}\
		}\
	} while ( 0 )
...
#define ASSERT_CAM_ERROR( err )\
	...
	case CAM_ERROR_BUFFERUNAVAILABLE:\
		TRACE( CAM_ERROR, "Error Code: CAM_ERROR_BUFFERUNAVAILABLE, %s - %d\n", __FILE__, __LINE__ );\
		break;\
...
#define CHECK_CAM_ERROR( err )\
	...
	if ( err > CAM_ERROR_STARTCODE ) {return err;}\
	} while ( 0 )
