...
#ifdef NEW_POWEROPT_SOLUTION
#include "cpufreqd_xpu_clnt.h"
#include "cpufreqd_xpu_vmeta.h"
#endif
...
#define INVALID_SOCKET_NO (-1)
#define SOCKET_RETRY_TIMES 5
#define SOCKET_SLEEP_US 50000
static SIGN32 vmeta_socket[MAX_VMETA_INSTANCE] = {0};
...
SIGN32 vdec_os_driver_init(void)
{
	...
	/*set all the vmeta_socket[] to INVALID_SOCKET_NO*/
	for(i= 0; i < MAX_VMETA_INSTANCE; i++) {
		if(0 == vmeta_socket[i])
			vmeta_socket[i] = INVALID_SOCKET_NO;
	}
...
SIGN32 vdec_os_api_force_ini(void)
{
	...
	if (p_cb->kernel_share_va == 0) {
		if(VDEC_OS_DRIVER_OK != vdec_os_api_get_ks(&p_ks)) {
			dbg_printf(VDEC_DEBUG_ALL,
				"vdec_os_api_force_ini: init error\n");
			return -1;
		}
	} else {
...
SIGN32 vdec_os_api_get_user_id(void)
{
	...
	if (p_cb->kernel_share_va == 0) {
		if(VDEC_OS_DRIVER_OK != vdec_os_api_get_ks(&p_ks)) {
			dbg_printf(VDEC_DEBUG_ALL,
				"vdec_os_api_get_user_id: init error\n");
			return -1;
		}
	} else {
		p_ks = (kernel_share *) p_cb->kernel_share_va;
...
SIGN32 vdec_os_api_register_user_id(SIGN32 user_id)
{
	...
	if (p_cb->kernel_share_va == 0) {
		if(VDEC_OS_DRIVER_OK != vdec_os_api_get_ks(&p_ks)) {
			dbg_printf(VDEC_DEBUG_ALL,
				"vdec_os_api_register_user_id: init error\n");
			return VDEC_OS_DRIVER_USER_ID_FAIL;
		}
	} else {
		p_ks = (kernel_share *) p_cb->kernel_share_va;
	...
	p_ks->user_id_list[user_id].pt = (unsigned int)pthread_self();
	#if NEED_MONITOR
...
SIGN32 vdec_os_api_get_user_count(void)
{
	...
	if (p_cb->kernel_share_va == 0) {
		if(VDEC_OS_DRIVER_OK != vdec_os_api_get_ks(&p_ks)) {
			dbg_printf(VDEC_DEBUG_ALL,
				"vdec_os_api_get_user_count: init error\n");
			return -1;
		}
	} else {
		p_ks = (kernel_share *) p_cb->kernel_share_va;
...
resolution > 720p           -- 14
*/
static SIGN32 _vmeta_get_default_op(vmeta_user_info *p_info, SIGN32 *max_op)
{
	SIGN32 op = -1;
	int reso;
	...
	if (p_cb->kernel_share_va == 0) {
		if(VDEC_OS_DRIVER_OK != vdec_os_api_get_ks(&p_ks)) {
			dbg_printf(VDEC_DEBUG_POWER,
				"vdec_os_api_update_user_info: init error\n");
			return -VDEC_OS_DRIVER_UPDATE_FAIL;
		}
	} else {
		p_ks = (kernel_share *) p_cb->kernel_share_va;
...
{
#ifdef NEW_POWEROPT_SOLUTION
	ALOGD("%s() ret 1, new power solution", __FUNCTION__);
	return 1;
#else
	ALOGD("%s() ret 0", __FUNCTION__);
	return 0;
#endif
}
...
{
#ifdef NEW_POWEROPT_SOLUTION
	struct xpu_vmeta_info xpu_info;
	int ret;
	int i;

	/*invalid user_id*/
	if(user_id >= 32 || user_id < 0) {
		ALOGD("%s() VMETA_INSTANCE_OVERANGE\n", __FUNCTION__);
		return -VMETA_INSTANCE_OVERANGE;
	}

	/*option 0: create; option 1: update; option 2: close*/
	switch(option) {
	/*create stage, create socket for cpufreqd, and send the 1st packet*/
	case(0):
		ALOGD("UserID %d: app %d, perf %d, w %d, h %d, ctype %d, strm %d", user_id, info->app_type,
			 info->perf_req, info->width, info->height, info->codec_type, info->strm_fmt);
		/*in some case, the socket was not properly closed last time,
		 so close it here*/
		if(vmeta_socket[user_id] != INVALID_SOCKET_NO) {
			cpufreq_xpu_close(vmeta_socket[user_id]);
			vmeta_socket[user_id] = INVALID_SOCKET_NO;
		}

		for(i = 0; i < SOCKET_RETRY_TIMES; i++) {
			vmeta_socket[user_id] = cpufreq_xpu_open();
			if(vmeta_socket[user_id] != INVALID_SOCKET_NO)
				break; /*break out of the for loop*/
			usleep(SOCKET_SLEEP_US);
		}
		if(vmeta_socket[user_id] == INVALID_SOCKET_NO) {
			ALOGD("%s() failed to open socket!\n", __FUNCTION__);
			return -SOCKET_CANNOT_OPEN;
		}

		/*no break here, continute to next case*/

	/*update stage, send packet to cpufreqd*/
	case(1):
		xpu_info.head.flags = XPU_USR_VMETA;

		if(info->codec_type == 0)
			xpu_info.func = VMETA_DECODE;
		else if(info->codec_type == 1)
			xpu_info.func = VMETA_ENCODE;
		else
			xpu_info.func = VMETA_IDLE;

		xpu_info.width = info->width;
		xpu_info.height = info->height;
		xpu_info.bias = info->perf_req;

		/*h263 and vc1 format video require MEDIUM, other format require LOW*/
		if(info->strm_fmt == 4 || info->strm_fmt == 10)
			xpu_info.stream = VMETA_STREAM_MEDIUM;
		else
			xpu_info.stream = VMETA_STREAM_LOW;

		if(info->app_type == 0) /*info->app_type 0:fullspeed, 1:not-fullspeed*/
			xpu_info.high = 1; /*xpu_info.high 1: full speed*/
		else
			xpu_info.high = 0;

		/*length of reserved area is -0- "void* reserved[0]"*/
		/*memset(&(xpu_info.reserved), 0, sizeof(xpu_info.reserved));*/
		...
		for(i = 0; i < SOCKET_RETRY_TIMES; i++) {
			...
			if(vmeta_socket[user_id] != INVALID_SOCKET_NO) {
				ret = cpufreq_xpu_update(vmeta_socket[user_id],
					&(xpu_info.head), sizeof(struct xpu_vmeta_info));
				if(ret == sizeof(struct xpu_vmeta_info))
					break; /*break out of the for loop*/
				cpufreq_xpu_close(vmeta_socket[user_id]);
				vmeta_socket[user_id] = INVALID_SOCKET_NO;
			}
		...
		if(ret != sizeof(struct xpu_vmeta_info)) {
			ALOGD("%s() failed to send socket message!\n", __FUNCTION__);
			return -SOCKET_CANNOT_SENDMSG;
		}

		break;

	/*close socket. For abnormal exit case, this stage probably isn't called in user-space*/
	case(2):
		ALOGD("UserID %d will close socket", user_id);
		if(vmeta_socket[user_id] != INVALID_SOCKET_NO) {
			cpufreq_xpu_close(vmeta_socket[user_id]);
			vmeta_socket[user_id] = INVALID_SOCKET_NO;
		}
	...
	/*unspported operations*/
	default:
		ALOGD("%s() UNSUPPORTED_OPERATION\n", __FUNCTION__);
		return -UNSUPPORTED_OPERATION;
	}

	ALOGD("%s(id:%d, perf:%d) ret no error", __FUNCTION__, user_id, info->perf_req);
	return 0;
#else
	return 0;
#endif
