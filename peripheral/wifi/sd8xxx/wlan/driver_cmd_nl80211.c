...
static int wpa_driver_set_backgroundscan_params(void *priv, char *cmd) {
    ...
    if (!find_ssid) {
        while ((i < NL80211_SSID_AMOUNT) && (ssid_conf != NULL)) {
            if ((!ssid_conf->disabled) &&
                (ssid_conf->ssid_len <= NL80211_SSID_MAX_SIZE)) {
...
int wpa_driver_nl80211_driver_cmd(void *priv, char *cmd, char *buf, size_t buf_len) {
    ...
    if (os_strcasecmp(cmd, "STOP") == 0) {
        ...
        os_memcpy(&priv_cmd.buf, &buf, sizeof(buf));
        ...
        *pbuf++ = '\0';
        ...
        if (ap_wps_p2p_ie) {
            if ((size_t) (buf + buf_size - pbuf) < wpabuf_len(ap_wps_p2p_ie)) {
                ...
                wpa_printf(MSG_DEBUG, "Realloc WPS P2P IE buffer size to %u bytes", buf_size);
            }
