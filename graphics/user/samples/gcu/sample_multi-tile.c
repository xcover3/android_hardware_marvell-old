...
#endif
{
    ...
    pSrcARGBSurface = _gcuLoadRGBSurfaceFromFile(pContextA, "ARGB8888-multi-tiled.bmp");
    for(i = 0; i < iF; i++)
    {
        ...
        surfaceData.flag.bits.tileType           = 3;
        ...
        strcpy(filename1, "multi_tile");
        strcat(filename1, sFormat[i]);
        //_gcuDumpSurface(pContextA, pSrcMultiTileSurface, filename1);
        ...
        for(j = 0; j < iF; j++)
        {
            ...
            //blit multi-tiled aFormat[i] => tiled aFormat[j]
            memset(&bltData, 0, sizeof(bltData));
            ...
            //blit multi-tiled aFormat[i] => linear aFormat[j]
            memset(&bltData, 0, sizeof(bltData));
            bltData.pSrcSurface = pSrcMultiTileSurface;
            bltData.pDstSurface = pDstLinearSurface;
            bltData.pSrcRect = &srcRect;
