...
#endif
{
    ...
    pSrcARGBSurface = _gcuLoadRGBSurfaceFromFile(pContextA, "ARGB8888-supertiled-2.bmp");
    for(i = 0; i < iF; i++)
    {
        ...
        surfaceData.flag.bits.tileType           = 2;
        ...
        for(j = 0; j < iF; j++)
        {
            ...
            //blit super-tiled aFormat[i] => tiled aFormat[j]
            memset(&bltData, 0, sizeof(bltData));
            ...
            //blit super-tiled aFormat[i] => linear aFormat[j]
            memset(&bltData, 0, sizeof(bltData));
            bltData.pSrcSurface = pSrcMultiTileSurface;
            bltData.pDstSurface = pDstLinearSurface;
            bltData.pSrcRect = &srcRect;
