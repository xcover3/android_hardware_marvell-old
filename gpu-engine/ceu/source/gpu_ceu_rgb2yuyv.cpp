...
static const char gStrFragShader[] =
    ...
    "  gl_FragColor.x = dot(rgb2y, p1rgb);\n"
    "  gl_FragColor.y = dot(rgb2u, p1rgb);\n"
    "  gl_FragColor.z = dot(rgb2y, p2rgb);\n"
    "  gl_FragColor.w = dot(rgb2v, p1rgb);\n"
    "}                             \n";
...
/* Internal function prototype for rgb2yuyv.*/
ceuSTATUS _rgb2yuyv_set_vertex_data(CEUContext* pContext);

ceuSTATUS gpu_ceu_rgb2yuyv_init(CEUContext* pContext)
{
    gpu_rgb2yuyv_t* pEffectData = NULL;
    if(GPU_VIR_NULL != pContext->effectParam[GPU_RGB2YUYV])
    {
        /*Already Initialized.*/
        pEffectData = (gpu_rgb2yuyv_t*)pContext->effectParam[GPU_RGB2YUYV];

        glUseProgram(pEffectData->programObject);
        _rgb2yuyv_set_vertex_data(pContext);

        return ceuSTATUS_SUCCESS;
    }

    pEffectData = (gpu_rgb2yuyv_t*)gpu_ceu_allocate(sizeof(gpu_rgb2yuyv_t));
    ...
    pContext->effectParam[GPU_RGB2YUYV] = (void*)pEffectData;
    ...
    TIME_END("rgb2yuyv Shader Compile");
...
ceuSTATUS gpu_ceu_rgb2yuyv_deinit(CEUContext* pContext)
{
    ...
    gpu_rgb2yuyv_t* pEffectData = (gpu_rgb2yuyv_t*)pContext->effectParam[GPU_RGB2YUYV];
    ...
    gpu_ceu_free(pContext->effectParam[GPU_RGB2YUYV]);
    pContext->effectParam[GPU_RGB2YUYV] = GPU_VIR_NULL;

    return ceuSTATUS_SUCCESS;
}

ceuSTATUS _rgb2yuyv_set_vertex_data(
    CEUContext* pContext
)
{
    gpu_rgb2yuyv_t* pEffectData = (gpu_rgb2yuyv_t*)pContext->effectParam[GPU_RGB2YUYV];
