...
namespace android {
    ...
    int FakeCam::getNumberOfCameras(){
        ...
        Param[0].setPreviewFormat("yuv420sp");
        Param[1].setPreviewFormat("yuv420sp");
        Param[0].set(CameraParameters::KEY_VIDEO_FRAME_FORMAT,"yuv420sp");
        Param[1].set(CameraParameters::KEY_VIDEO_FRAME_FORMAT,"yuv420sp");

        return sizeof(sCameraInfo) / sizeof(sCameraInfo[0]);
    }

    void FakeCam::getCameraInfo(int cameraId, struct camera_info* cameraInfo)
    {
    ...
    FakeCam::~FakeCam(){
        mPreviewBufQueued.clear();
        mPreviewBufDequeued.clear();
    }

    void FakeCam::setCallbacks(sp<CameraMsg>& cameramsg,
    ...
    void FakeCam::setCallbacks(camera_notify_callback notify_cb,camera_data_callback data_cb,
            camera_request_memory get_camera_memory,void* user){
    }
    ...
    status_t FakeCam::startFaceDetection(){
        return NO_ERROR;
    }

    status_t FakeCam::stopFaceDetection(){
        return NO_ERROR;
    }
    ...
    const CameraSetting& FakeCam::getParameters(){
        return mSetting;
    }

    /*
    status_t FakeCam::registerPreviewBuffers(sp<BufferHolder> imagebuf){
    ...
    */
    ...
    status_t FakeCam::enqPreviewBuffer(sp<CamBuf> buf){
        mPreviewBufQueued.push_back(buf);
        return NO_ERROR;
    }

    status_t FakeCam::deqPreviewBuffer(void** key){
        ...
        int cnt = mPreviewBufQueued.size();
        if(cnt > 0){
            List<sp<CamBuf> >::iterator itor = mPreviewBufQueued.begin();
            fillpreviewbuffer(*itor);
            mPreviewBufDequeued.push_back(*itor);
            *key = (void*)((*itor)->getKey());
            mPreviewBufQueued.erase(itor);
            return NO_ERROR;
        }
    ...
    /*
    status_t FakeCam::registerStillBuffers(sp<BufferHolder> imagebuf){
    ...
    */
    ...
    status_t FakeCam::unregisterStillBuffers(){
        mStillBufQueued.clear();
        mStillBufDequeued.clear();
        return NO_ERROR;
    }

    status_t FakeCam::enqStillBuffer(sp<CamBuf> buf){
        mStillBufQueued.push_back(buf);
        return NO_ERROR;
    }

    status_t FakeCam::deqStillBuffer(void** key){
        int cnt = mStillBufQueued.size();
        if(cnt > 0){
            if(mMsg->msgTypeEnabled(CAMERA_MSG_SHUTTER)){
               TRACE_D("%s:shutter close",__FUNCTION__);
                {
            ...
            List<sp<CamBuf> >::iterator itor = mStillBufQueued.begin();
            fillstillbuffer(*itor);
            mStillBufDequeued.push_back(*itor);
            *key = (void*)((*itor)->getKey());
            mStillBufQueued.erase(itor);
            return NO_ERROR;
        }
        /*
    ...
    EngBufReq FakeCam::getPreviewBufReq(){
        const char* format = mSetting.getPreviewFormat();
        int w,h;
        mSetting.getPreviewSize(&w, &h);
        EngBufReq bufreq = EngBufReq::getEngBufReq(format, w, h, 2);
        return bufreq;
    }

    EngBufReq FakeCam::getStillBufReq(){
    ...
    sp<IMemory> FakeCam::getExifImage(const void* key)
    {
        return NULL;
    }
    ...
    //-------------------------------------------------
    void FakeCam::fillpreviewbuffer(sp<CamBuf> imagebuf){
        static char timer=0;
        timer++;
        const unsigned char* bufc = imagebuf->getVAddr();
        unsigned char* buf = const_cast<unsigned char*>(bufc);
        int len = imagebuf->getLen();
        memset(buf,timer,len);
    }
     ...
     */
    void FakeCam::fillstillbuffer(sp<CamBuf> imagebuf){
        const unsigned char* bufc = imagebuf->getVAddr();
        unsigned char* buf = const_cast<unsigned char*>(bufc);
        ...
        if (fptr == 0) {
            ...
            memcpy((void*)buf,(void*)internaljpeg, sizeof(internaljpeg));
        }
        else{
            TRACE_D("%s: use user specified file for jpeg callback, %s", __FUNCTION__, fname);
            pic_len = fread((void*)buf, 1, buf_len, fptr);
            fclose(fptr);
        }
    }
