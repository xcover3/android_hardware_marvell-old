...
#define   _ALIGN_TO(x,iAlign) ( (((Ipp32s)(x)) + (iAlign) - 1) & (~((iAlign) - 1)) )
    ...
    static IppStatus _ipp_jpeg_thumbnail(const CAM_ImageBuffer *pSrcImgBuf, CAM_ImageBuffer *pDstImgBuf, Ipp32s iQualityFactor )
    {
        ...
        else if(CAM_IMGFMT_RGBA8888 == pSrcImgBuf->eFormat) {
            TRACE_E("%s: thumbnail src image is RGBA8888",__FUNCTION__);
            memcpy(&stTempBuffer, pSrcImgBuf, sizeof(CAM_ImageBuffer));
        }
        else if(CAM_IMGFMT_YCbYCr == pSrcImgBuf->eFormat) {
            TRACE_E("%s: thumbnail src image is YUYV",__FUNCTION__);
            memcpy(&stTempBuffer, pSrcImgBuf, sizeof(CAM_ImageBuffer));
        }
        TRACE_E("ppu format is :%d", pSrcImgBuf->eFormat);
        ...
        if( CAM_IMGFMT_JPEG == pDstImgBuf->eFormat)
        {
            ...
            stSrcPicture.picFormat           = JPEG_YUV422I;
            ...
            TRACE_E("picWidth:%d picHeight:%d stSrcPicture.picPlaneStep[0] %d stSrcPicture.ppPicPlane[0] %x", 
                    stSrcPicture.picWidth, stSrcPicture.picHeight, stSrcPicture.picPlaneStep[0], stSrcPicture.ppPicPlane[0]);
    ...
    IppExifFlash updateFlashInfo(IppExifInfo *pExifInfo,const CameraSetting& camsetting,
            const CAM_ImageBuffer* const pImgBuf_still)
    {

        IppExifFlash flashInfo = ippExifNoFlashFunction;
        if (pImgBuf_still->bEnableShotInfo)
        {
            const char* flashmodes = camsetting.get(CameraParameters::KEY_SUPPORTED_FLASH_MODES);
            if (0 == flashmodes)
            {
                flashInfo  = ippExifNoFlashFunction;
            }
            else
            {
                const char* currentflashmode = camsetting.get(CameraParameters::KEY_FLASH_MODE);
                if (0 != currentflashmode)
                {
                    int setting = camsetting.map_andr2ce(CameraSetting::map_flash, currentflashmode);
                    bool bFlashFired = pImgBuf_still->stShotInfo.bIsFlashOn;
                    if (-1 != setting)
                    {
                        if (setting == CAM_FLASH_AUTO)
                        {
                            if (bFlashFired)
                                flashInfo = ippExifFlashFired_ATMODE;
                            else
                                flashInfo = ippExifFlashNotFired_ATMODE;
                        }
                        else if (setting == CAM_FLASH_OFF)
                        {
                            flashInfo = ippExifFlashNotFired_CFMODE;
                        }
                        else if (setting == CAM_FLASH_ON)
                        {
                            flashInfo = ippExifFlashFired_CFMODE;
                        }
                        else if (setting == CAM_FLASH_TORCH)
                        {
                            flashInfo = ippExifFlashFired;
                        }
                        else if (setting == CAM_FLASH_REDEYE)
                        {
                            flashInfo = ippExifFlashFired_RDMODE;
                        }
                        else
                        {
                            //unsupport flash mode;
                            flashInfo = ippExifFlashNotFired;
                        }
                    }
                    else
                    {
...
#define _LOG2(x) (log((double)(x)) / log(2.0))
            ...
            pExifInfo->stExifPrivateTagInfo.eFlash              = updateFlashInfo(pExifInfo, setting, pImgBuf_still);
        ...
        //for GPS information
        const char *cLatitude  = setting.get(CameraParameters::KEY_GPS_LATITUDE);
        if (0 != cLatitude)
        {
            const char *cLongitude = setting.get(CameraParameters::KEY_GPS_LONGITUDE);
            const char *cAltitude  = setting.get(CameraParameters::KEY_GPS_ALTITUDE);

            double latitude  = strtod(cLatitude, NULL);
            double longitude = strtod(cLongitude, NULL);
            double altitude  = strtod(cAltitude, NULL);
	     ...
	     pExifInfo->stGPSInfo.eGPSAltitudeRef            = (altitude > 0) ? ippExifAltitudeRefSeaLevel : ippExifAltitudeRefSeaLevelNeg;
            ...
            pExifInfo->stGPSInfo.urGPSLatitute[2].numerator     = (uint32_t)((((latitude - pExifInfo->stGPSInfo.urGPSLatitute[0].numerator) * 60)
                        - pExifInfo->stGPSInfo.urGPSLatitute[1].numerator) * 60 * 1000);
            pExifInfo->stGPSInfo.urGPSLatitute[2].denominal     = 1000;
            ...
            pExifInfo->stGPSInfo.urGPSLongtitute[2].numerator   = (uint32_t)((((longitude - pExifInfo->stGPSInfo.urGPSLongtitute[0].numerator) * 60)
                        - pExifInfo->stGPSInfo.urGPSLongtitute[1].numerator) * 60 * 1000);
            pExifInfo->stGPSInfo.urGPSLongtitute[2].denominal   = 1000;
            ...
            long lTimestamp = strtol(sTimestamp, NULL, 10);
            ...
            //date stamp
            strftime((char *)pExifInfo->stGPSInfo.ucGPSDateStamp, 11, "%Y:%m:%d", &tmTimestamp);
        }
        ...
        if(NULL ==pThumbnailBuf){
            ...
            pExifInfo->stThumbnailInfo.eOrientation = pExifInfo->stTiffInfo.eOrientation;
    ...
    status_t ExifGenerator(const CAM_ImageBuffer* const pImgBuf_still, const CameraSetting& setting, sp<CamBuf>& output)
    {
...
#undef SUPPORT_INTERLEAVE_JPEG
        ...
        if(ThumbnailBuf.iWidth <= 0 || ThumbnailBuf.iHeight <= 0 ||
            ...
            int thumblen=ThumbnailBuf.iWidth * ThumbnailBuf.iHeight * 4 + 128;
...
#else
        Vector<sp<CamBuf> > queue = BufferHolder::allocRawBuffersWithAllocator(
                (size_t)cBufReq.iMinBufLen + srcImgBuf_len,
                1);
        if((int)(queue.size()) <= 0){
            TRACE_E("allocate mem fail");
            return -1;
        }
        exif_mem = queue.itemAt(0);
        {
            int w,h;
            exif_mem->setSize(pImgBuf_still->iWidth, pImgBuf_still->iHeight);
            exif_mem->setFormat(CameraParameters::PIXEL_FORMAT_JPEG);
        }
        uint8_t* ptr_exif = (uint8_t*)(exif_mem->getVAddr());
        //TODO:we may ignore memset here for perf.
        memset(ptr_exif, 0, exif_mem->getLen());

        //exif dest buffer info
        IppExifImgBuf cDesImg;
        cDesImg.pBuffer=ptr_exif;
        cDesImg.iFilledLen=exif_mem->getLen();
#endif
