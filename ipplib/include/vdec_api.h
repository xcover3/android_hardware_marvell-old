...
#define VMETA_RC_SLICE_MISS 		     30006  // Not a fatal error; slice missing for non-ASO streams.
...
#define VDEC_RC_SLICE_MISS              VMETA_RC_SLICE_MISS
...
TYPEDEF_STRUCT(vdec_strm_config_t)
{
	...
	unsigned int    p_frame_seek;           //!< setting for P Frame seek (all intra mb)
											//      0=disable; 1=enable 
	...
	unsigned int    output_420P;			// output mode, 0: uyvy, 1 420P
	unsigned int    no_res_change;			// 0:resolution change allowd, 1: resolution change forbidden
	
	unsigned char   force_ref_out;    		// 1: output reference buffer even for non-refence picture;  
	unsigned char   disable_dis_out;        // 1: don't output display buffer;  
...
TYPEDEF_STRUCT(venc_strm_config_t)
{
    ...
    unsigned int    profile_idc;            //!< H.264 profile
