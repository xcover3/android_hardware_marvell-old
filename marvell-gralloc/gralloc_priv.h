...
#define GC_PRIVATE_HANDLE_INT_COUNT    34
...
struct private_handle_t {
    ...
    static int validate(const native_handle* h)
    {
                ...
                hnd->magic != sMagic)
        {
            ...
            ALOGE("handle:%p, version:0x%x, numInts:%d, numFds:%d, magic:0x%x",
                h, h->version, h->numInts, h->numFds, hnd->magic);
            ALOGE("target version:0x%x, numInts:%d, numFds:%d, magic:0x%x",
                sizeof(native_handle), GC_PRIVATE_HANDLE_INT_COUNT, GC_PRIVATE_HANDLE_FD_COUNT,
                sMagic);
            return -EINVAL;
        }
        return 0;
