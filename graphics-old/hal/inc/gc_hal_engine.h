...
gceSTATUS gcoHAL_QueryPixelPipesInfo(
...
/* Upload data to an gcoTEXTURE object, "Memory" is physical address. */
gceSTATUS
gcoTEXTURE_Upload_Physical(
    IN gcoTEXTURE Texture,
    IN gctUINT MipMap,
    IN gceTEXTURE_FACE Face,
    IN gctUINT X,
...
/* Get texture width and height for a mipmap level*/
gceSTATUS
gcoTEXTURE_GetMipMapInfo(
    IN gcoTEXTURE Texture,
    IN gctUINT MipMap,
...
gcoVERTEXARRAY_Construct(
    IN gcoHAL Hal,
    IN gctINT32 CacheSize,
    IN gctINT32 IndexCacheSize,
...
gcoVERTEXARRAY_Bind(
    ...
    IN OUT gctUINT * PrimitiveCount,
#endif
    IN OUT gctBOOL * VBODirty,
    IN gctBOOL EnableWClip
    );

gceSTATUS
gcoVERTEXARRAY_BindIndex(
    IN gcoVERTEXARRAY Vertex,
    IN gctSIZE_T Count,
    IN gceINDEX_TYPE IndexType,
    IN gcoINDEX IndexObject,
