...
{
    gcvWhiteList_UnLimited = 1,
    gcvWhiteList_Customize = 2
}gceWhiteList;
...
typedef struct _gcsPLS
{
    ...
    /* Workaround for chrome createContext setting states to wrong HW issue.*/
    gctBOOL                     bChrome;
...
\******************************************************************************/
#if MRVL_ENABLE_USER_LEAK_CHECK
typedef struct _OsMemRecord * OsMemRecord_PTR;
typedef struct _OsMemRecord
{
    gctSIZE_T  Bytes;
    gctPOINTER Memory;
    gctUINT32  tid;
    gctCHAR    funcName[128];
    gctINT     line;
    OsMemRecord_PTR next;
}OsMemRecord;
#endif
...
/*Check whether support NP2 texture*/
gctBOOL
...
#define gcoOS_Allocate(Os, Bytes, Memory) \
    _gcoOS_Allocate(Os, Bytes, Memory, __FUNCTION__, __LINE__)
#define gcoOS_AllocateC(Os, Bytes, Memory,__FUNCTION__,__LINE__) \
    _gcoOS_Allocate(Os, Bytes, Memory, __FUNCTION__, __LINE__)
#endif
...
#   define _gcmVERIFY_ARGUMENT(prefix, arg) \
       ...
       { \
           if (!(arg)) \
           { \
               prefix##TRACE(gcvLEVEL_ERROR, #prefix "VERIFY_ARGUMENT failed: %s (%d)", __func__, __LINE__); \
               prefix##ASSERT(arg); \
